...
public class PhotoDaoImp implements PhotoDao {
	...
	public boolean delete(Long userid) {
		try {
			...
			Query query = session
					.createQuery("delete from Photo where userid=?");
			query.setParameter(0, userid);
			query.executeUpdate();
	...
	public boolean delete(Long userid, Integer type) {
		try {
			...
			Transaction ts = session.beginTransaction();
			Query query = session
					.createQuery("delete from Photo where userid=? and type=?");
			query.setParameter(0, userid);
	...
	public boolean delByUrl(String url) {
		try {
			Session session = HibernateSessionFactory.getSession();
			Transaction ts = session.beginTransaction();
			Query query = session.createQuery("delete from Photo where url=?");
