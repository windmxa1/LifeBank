	...
	public void login() {
		try {
			...
			if (u != null) {
				...
				message.put("username", uDetailDao.getDetail(u.getId())
						.getUsername());
	...
	public void changePw() {
		try {
			...
			if (user == null) {
				...
				if (uDao.update(user)) {
					message.put("username", uDao2.getDetail(user.getId())
							.getUsername());
					message.put("message", "success");
		...
		try {
			...
			if (user == null) {
				...
				if (newPassword != null) {
					...
					if (uDao.update(user)) {
						UserDetailDao uDao2 = new UserDetailDaoImp();
						message.put("username", uDao2.getDetail(user.getId()).getUsername());
						message.put("message", "success");
						message.put("description", "密码修改成功");
					} else {
						message.put("message", "error");
